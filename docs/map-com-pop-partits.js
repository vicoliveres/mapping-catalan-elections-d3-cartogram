...
const color = d3.scaleOrdinal().range(['#ff3f8a', '#ffb523', '#f27200', '#c80a1e', '#701b73', '#f7df0e', '#4d94e3']).domain(['JxCat', 'ERC', "C's", 'PSC', 'CatComú', 'CUP', 'PP']);
...
const size = d3.scaleSqrt().range(isMobile ? [2.5, 45] : isTablet ? [3, 70] : [4, 105]);
const font = d3.scaleLinear().range(isMobile ? [5, 12] : isTablet ? [7, 19] : [10, 28]);
const color = d3.scaleOrdinal().range(['#ff3f8a', '#ffb523', '#fa7600', '#c80a1e', '#701b73', '#f7df0e', '#4d94e3']).domain(['JxCat', 'ERC', "C's", 'PSC', 'CatComú', 'CUP', 'PP']);
  ...
  rect
    ...
    .on("mousemove", function(d) {
      ...
      tooltip.transition()
          ...
          .style("top", (d3.event.pageY - 28) + "px");
          tooltip.html("<b>" + namelong.get(d.properties.CODICOMAR) + "</b><br/>"
                  + "Change from 2015: " + canvipast.get(d.properties.CODICOMAR) + "</br>"
                  + "(+ indy / - union)");
        })
    .on("mouseout", function(d) {
      d3.select(this)
